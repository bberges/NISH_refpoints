...
setwd("~/Herring/Referenc points/antest002/run")
load("model.RData")
load("data.RData")
load("retro.RData")
load("forecast.RData")
...
fit$data$natMor <- fit$data$natMor[ac(1980:2023),]
...
catch.wt(stk) <- FLQuant(as.matrix(t(fit$data$catchMeanWeight)), dimnames=list(age=ages,year=year.range))
...
harvest.spwn(stk) <- FLQuant(as.matrix(t(fit$data$propF)), dimnames=list(age=ages,year=year.range))
